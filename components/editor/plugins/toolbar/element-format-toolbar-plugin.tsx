		...
		<>
			<ToggleGroup
				...
				{Object.entries(ELEMENT_FORMAT_OPTIONS).map(([value, option]) => (
					<ToggleGroupItem key={value} value={value} aria-label={option.name}>
						{option.icon}
					</ToggleGroupItem>
				))}
			</ToggleGroup>
			<ToggleGroup
				...
				value={elementFormat}
				onValueChange={handleValueChange}>
				<ToggleGroupItem value='outdent' aria-label='Outdent'>
					<IndentDecreaseIcon className='size-4' />
				</ToggleGroupItem>
				<ToggleGroupItem value='indent' aria-label='Indent'>
